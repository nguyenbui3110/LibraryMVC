...
public class LibraryItemProfile : Profile
{
    public LibraryItemProfile()
    {
        ...
        CreateMap<LibraryItemModel, Book>().ReverseMap()
            .ForMember(dest => dest.Type, opt => opt.MapFrom(src => ItemType.Book));
        CreateMap<LibraryItemModel, Dvd>().ReverseMap()
            .ForMember(dest => dest.Type, opt => opt.MapFrom(src => ItemType.Dvd));
        CreateMap<LibraryItemModel, Magazine>().ReverseMap()
            .ForMember(dest => dest.Type, opt => opt.MapFrom(src => ItemType.Magazine));
    }
}